...
const Index = () => {
  ...
  const [ctx, setCtx] = useState<{ name?: string; emergencyName?: string; emergencyPhone?: string }>({});

  const handleResult = (r: RiskResult, c: { name: string; emergencyName?: string; emergencyPhone?: string }) => {
    setResult(r);
    setCtx(c);
          ...
          <div className="animate-slide-up">
            ...
            <div className="flex flex-wrap gap-2 mt-6">
              {[{ icon: <HeartPulse className="w-4 h-4" />, label: "Heart Health" },
                ...
                { icon: <Shield className="w-4 h-4" />, label: "Emergency SOS" }].map((b) => (
